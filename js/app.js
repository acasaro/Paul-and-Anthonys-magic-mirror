...
var App = function () {
    ...
    var loadConfig = function (callback) {
        ...
        const envArgIndex = process.argv.indexOf("--profile");
        const env = envArgIndex > -1 ? process.argv[envArgIndex + 1] : null;
        let ob = { env, envArgIndex, argv: process.argv, new: process.argv[envArgIndex + 1] };
        try {
            fs.accessSync(configFilename, fs.F_OK);
            var c = require(configFilename);
            checkDeprecatedOptions(c);
            var config = Object.assign(defaults, c);
            callback(config, config.configName);
        } catch (e) {
            if (e.code === "ENOENT") {
                console.error(
                    Utils.colors.error(
                ...
                console.error(
                    Utils.colors.error(
                        "WARNING! Could not validate config file. Starting with default configuration. Please correct syntax errors at or above this line: " +
                            e.stack
                    )
                );
            } else {
